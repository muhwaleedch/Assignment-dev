...
public class JWTUtil {
    ...
    private String createToken(Map<String, Object> claims, String subject) {
        return Jwts.builder().setClaims(claims).setSubject(subject).setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * applicationProperties.getSessionExpiryTimeInMin()))
                .signWith(SignatureAlgorithm.HS256, SECRET_KEY).compact();
    }

    public Boolean validateToken(String token, UserPrincipal userDetails) {
        final String userUserId = extractUsername(token);
        return (userUserId.equals(userDetails.getUsername()) && !isTokenExpired(token));
    }
